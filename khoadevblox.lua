...
-- Định nghĩa các biến UI ở ngoài để các phần code phía dưới truy cập được
local Window
local v484, v485, v486, v487, v491, v489, v497, v493, v499, v494, v495, v496
local redzlib

local success, err = pcall(function()
    Window = WindUI:CreateWindow({
        Title = "Night Slayer Hub [V7]",
        Folder = "NightSlayerOnTop",
        Icon = "solar:settings-bold-duotone",
        NewElements = true,
        HideSearchBar = false,
        OpenButton = {
            Title = "Open Night Slayer Hub",
    ...
    local iconMap = {
        ...
        swords = "solar:sword-bold",
        cherry = "solar:cherry-bold",
        waves = "solar:water-bold",
        Signal = "solar:graph-bold",
        locate = "solar:map-point-bold",
        Scroll = "solar:document-bold",
        user = "solar:user-bold",
        shoppingCart = "solar:shop-bold",
        settings = "solar:settings-bold",
    }

    local function wrapTab(windTab)
        local wrapper = {}
        wrapper.wind_tab = windTab
        
        function wrapper:AddSection(arg)
            local title = ""
        ...
        function wrapper:AddDropdown(config)
            local title = config.Title or config.Name or ""
            local desc = config.Description or config.Desc or ""
        ...
        function wrapper:AddDiscordInvite(config)
            local title = config.Title or config.Name or "Discord Server"
            local desc = config.Description or config.Desc or ""
            local inviteUrl = config.Invite or ""
    ...
    local v466 = {}
    function v466:MakeTab(tabInfo)
        local name = tabInfo[1] or "Tab"
        local iconKey = tabInfo[2] or ""
        local icon = iconMap[iconKey] or iconKey
        if icon == "" or not string.find(tostring(icon), ":") or string.find(tostring(icon), "rbxassetid") then
    ...
    function v466:AddMinimizeButton(config)
    end

    redzlib = vu32

    v484 = v466:MakeTab({"Information", "info"})
    v485 = v466:MakeTab({"Farm", "home"})
    v486 = v466:MakeTab({"Fishing", "rbxassetid://127664059821666"})
...
if not success then
    warn("WindUI Boot Error: " .. tostring(err))
    pcall(function()
        ...
        tb.Size = UDim2.new(1, 0, 0, 100)
        tb.Position = UDim2.new(0, 0, 0, 50)
        tb.Text = "WindUI Boot Error: " .. tostring(err)
